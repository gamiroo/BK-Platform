...
- no stack traces in responses (any env)
- optional `error.details` allowed ONLY for safe, structured, non-sensitive diagnostics
- `x-request-id` header is always present
...
### 0.3 Authentication Defaults by Surface

BalanceGuard applies default authentication rules **only when identity resolution is active**:

| Surface | resolveActor present | Default auth required |
|-------  | ---------------------|-----------------------|
| site    | yes                  | ❌ No                 |
| client  | yes                  | ✅ Yes                |
| admin   | yes                  | ✅ Yes                |

If `requireAuth` is explicitly provided in route options, it overrides defaults.

If `resolveActor` is **not** provided, BalanceGuard operates in Day-0 public mode and does not enforce auth.

Additional invariant:

- Default auth rules apply ONLY when `resolveActor` is present.
- If `resolveActor` is absent, BalanceGuard operates in Day-0 public mode and does not enforce auth unless `requireAuth` is explicitly set.

### 0.4 Route Path Convention (Framework-Free Router)

Routes are registered using exact pathname matches.

- If the frontend calls `/api/client/auth/me`, the router MUST register the full path `/api/client/auth/me`.
- Vercel “surface handlers” may normalize prefixes before routing, but the in-process router remains exact-match and deterministic.
...
### 11.1 Origin enforcement (fail-closed in prod)

When `requireOrigin=true`, BalanceGuard MUST enforce an allowlisted Origin policy.

Failure modes MUST return `403 ORIGIN_REJECTED` with safe details:

- Missing `Origin` header:
  - `error.details.reason = "missing_origin"`
- Origin not in allowlist:
  - `error.details.reason = "origin_not_allowlisted"`
  - `error.details.origin = "<origin>"` (optional; safe)
- Allowlist missing/misconfigured in production:
  - `error.details.reason = "no_allowlist_configured"`

`error.details.surface` MAY be included to aid debugging.

### 11.2 CORS headers are surface-aware

CORS behavior MUST remain surface-aware and must not broaden access beyond the Origin policy.
Preflight (`OPTIONS`) MUST short-circuit before origin/csrf/rate limiting.
...
### 13.1 Error Envelope Shape (Canonical)

All non-2xx responses MUST use a consistent JSON envelope:
...
```json
{
  ...
  "error": {
    ...
    "request_id": "uuid",
    "details": { "optional": "safe diagnostics" }
  }
}
```

### 13.2 Error Details Policy (Non-Leaky Diagnostics)

BalanceGuard MAY attach `error.details` for *expected* failures where it improves developer/operator clarity without increasing risk.

Examples of allowed details:

- `ORIGIN_REJECTED`
  - `surface`
  - `reason` (e.g. `no_allowlist_configured`)
- `WRONG_SURFACE`
  - `expected_cookie` (cookie name only, not cookie value)
  - `got_cookie` (cookie name only, not cookie value)

Non-negotiables:

- Never include stack traces, raw error objects, or exception names in client responses.
- Never include cookie values, tokens, user identifiers, emails, or request bodies in `details`.
- For `INTERNAL_ERROR`, return a generic message only (no `details`).

### 13.3 Canonical Error Codes

| Code | HTTP | Meaning |
| ---- | ---- | ------- |
| AUTH_REQUIRED | 401 | No valid session (anon actor when auth required) |
| FORBIDDEN | 403 | Role not permitted |
| WRONG_SURFACE | 403 | Session cookie present but for the wrong surface |
| STEP_UP_REQUIRED | 403 | AAL insufficient |
| ORIGIN_REJECTED | 403 | Origin missing/not allowlisted or allowlist misconfigured |
| CSRF_INVALID | 403 | CSRF failed |
| RATE_LIMITED | 429 | Too many requests |
| NOT_FOUND | 404 | No route match |
| METHOD_NOT_ALLOWED | 405 | Wrong method for an existing endpoint |
