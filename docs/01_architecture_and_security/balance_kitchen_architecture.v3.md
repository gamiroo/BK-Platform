...
- **client**
  - Owns client authentication routes under the client API prefix:
    - `/api/client/auth/login`
    - `/api/client/auth/logout`
    - `/api/client/auth/me`
  - Owns session-bearing client APIs under `/api/client/*`

- **admin**
  - Owns admin authentication routes under the admin API prefix:
    - `/api/admin/auth/login`
    - `/api/admin/auth/logout`
    - `/api/admin/auth/me`
  - Owns privileged admin APIs under `/api/admin/*`
  
**Why the `/api/<surface>/...` prefix is canonical:**

In the Vercel `api` project, each surface is implemented as a catch-all function:

- `api/client/[...path].ts` → routes `/api/client/*` into the client surface router
- `api/admin/[...path].ts` → routes `/api/admin/*` into the admin surface router
- `api/site/[...path].ts`  → routes `/api/site/*` into the site surface router (where used)

This preserves strict surface separation while keeping deployments framework-free and deterministic.
...
### 13.3.1 Canonical Frontend HTTP Client (Mandatory)

All frontend HTTP requests MUST go through a single shared abstraction:
Direct usage of `fetch()` in UI code is **forbidden**.

#### Rationale

Balance Kitchen enforces strict transport and security invariants:

- surface-isolated authentication (client vs admin)
- HttpOnly session cookies
- CSRF enforcement
- Origin + CORS correctness
- standardized error envelopes
- request_id propagation
- future observability hooks (timing, retries, tracing)

These guarantees **cannot be enforced** if `fetch()` is used ad-hoc.

#### Rules (Non-Negotiable)

- UI code (`pages/`, `views/`, `components/`) MUST NOT call `fetch()`
- All HTTP requests MUST go through `http-client.ts`
- `http-client.ts` is the **only place** allowed to:
  - call `fetch()`
  - set `credentials: "include"`
  - parse JSON envelopes
  - normalize transport errors
- Any exception requires an architecture change
