// src/server/http/routes/admin/auth.login.post.ts
//
...
export const adminAuthLogin = balanceguardAdmin(
  ...
  async (ctx: RequestContext, req: Request) => {
    ...
    if (out.actor.kind !== "admin" || out.actor.role !== "admin") {
      throw new AppError({
        code: "WRONG_SURFACE",
        status: 400,
        message: "Wrong surface",
        details: { reason: "admin_login_actor_not_admin" },
      });
    }
    ...
    const base = json<AdminLoginResponse>(ctx, {
      actor: { kind: "admin", role: "admin", user_id: out.actor.user_id },
      csrf_token: csrfToken,
      csrf_cookie: csrfCookieName("admin"),
    });
